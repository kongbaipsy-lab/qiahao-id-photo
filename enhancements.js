...
let cetTarget=null;
function applyCet(w,h){cetTarget=[w,h];$('width').value=w;$('height').value=h;$('dpi').value='300';$('format').value='jpeg';$('limit').disabled=false;$('limit').value='200';$('domestic').value=w+','+h+',300,cet';updateSize();$('sizeHint').textContent='四六级：'+w+' × '+h+' px · JPG · 300 DPI · 200 KB 上限。底色和文件大小下限请按本校要求检查。';}
...
function isCetActive(){return !!cetTarget&&$('domestic').value.endsWith(',cet')}
function assertCetSettings(){if(W!==cetTarget[0]||H!==cetTarget[1])throw Error('当前尺寸与所选四六级预设不同，请重新选择预设。');if($('format').value!=='jpeg'||+$('dpi').value!==300||+$('limit').value===0||+$('limit').value>200)throw Error('四六级导出需要 JPG、300 DPI 和不超过 200 KB 的上限，请重新点击一键设置并处理。');if(!cutout||!['#ffffff','#64c5ff'].includes(bg))throw Error('四六级底色尚未处理完成，请选择纯白或参考浅蓝并点击一键设置并处理。');}
$('cetProcess').onclick=async()=>{if(!original){message('请先上传照片。',true);$('file').click();return}if(segmenting){message('正在抠图，请稍候。');return}const photo=original,target=cetTarget||[480,600];applyCet(...target);setBg($('cetBg').value);$('cetProcess').disabled=true;$('cetProcess').textContent='正在设置并处理…';try{await $('removeBG').onclick();if(photo!==original)return;if(!cutout||bg==='original')throw Error('抠图没有完成，请重试。');$('cetProgress').textContent='已设置 '+W+' × '+H+' px、JPG、300 DPI、200 KB 上限，并应用'+(bg==='#ffffff'?'白底':'浅蓝底')+'。下载时核验实际文件。';}catch(e){message(e.message,true)}finally{$('cetProcess').disabled=false;$('cetProcess').textContent='一键设置并处理照片'}};
async function inspectExport(b){const bitmap=await createImageBitmap(b),w=bitmap.width,h=bitmap.height;bitmap.close();const bytes=new Uint8Array(await b.arrayBuffer());const jpg=bytes[0]===255&&bytes[1]===216;let dpi=null;if(jpg){for(let i=2;i<bytes.length-16;){if(bytes[i]!==255)break;const marker=bytes[i+1];if(marker===0xda||marker===0xd9)break;const len=(bytes[i+2]<<8)|bytes[i+3];if(marker===0xe0&&String.fromCharCode(...bytes.slice(i+4,i+9))==='JFIF\0'&&bytes[i+11]===1){dpi=[(bytes[i+12]<<8)|bytes[i+13],(bytes[i+14]<<8)|bytes[i+15]];break}if(len<2)break;i+=2+len}}
 if(w!==W||h!==H)throw Error('导出像素校验失败，请重试。');
 if(isCetActive()){assertCetSettings();if(!jpg||!dpi||dpi[0]!==300||dpi[1]!==300)throw Error('JPG 或 DPI 校验失败，未下载。');if(b.size<20*1024)throw Error('最高质量导出仅 '+(b.size/1024).toFixed(1)+' KB，未达到当前参考要求的 20 KB 下限。请使用细节更丰富、清晰的原图；不会添加无效数据凑大小。');if(b.size>200000||b.size>+$('limit').value*1000)throw Error('文件超过所选大小上限，未下载。');}
 return '实际文件已核验：'+w+' × '+h+' px · '+(jpg?'JPG':'PNG')+(dpi?' · '+dpi.join(' × ')+' DPI':'')+'\n大小：'+b.size+' 字节（'+(b.size/1024).toFixed(1)+' KiB）\n底色：'+(cutout&&bg!=='original'?(bg==='#ffffff'?'已合成纯白底':bg==='#64c5ff'?'已合成参考浅蓝底':'已合成自定义底色'):'原图背景，未核验')+'；发丝、头部比例及学校审核要求需人工检查。';
}
